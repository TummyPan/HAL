...
public class VesselDensityTest extends ExampleCell {
    public static void main(String[]args) throws IOException {
        ...
        // name results folder
        String resultsFolder = "C:\\MyFiles\\PHD Data\\VesselDensityMeasure\\DensityTest";
        ...
        for (int k = 0; k < 1; k++) {
            ...
            for (int i = 0; i < 1; i++) {
                ...
                int timesteps = 20000; // number of iterations of model
...
*/
                ...
                double[] cancerPopulationQ1 = new double[timesteps];
                double[] cancerPopulationQ2 = new double[timesteps];
                double[] cancerPopulationQ3 = new double[timesteps];
                double[] cancerPopulationQ4 = new double[timesteps];
                ...
                // set up animation
                GridWindow agentLayer = new GridWindow(x, y, 2);
                GridWindow drugLayer = new GridWindow(x, y, 2);
                GridWindow proliferationLayer = new GridWindow(x, y, 2);
...
*/
                ...
                FileIO quadrantPopulations = new FileIO(resultsFolder + "\\Data\\QuadrantPopulations_" + runDateTime + "_run_" + k + "_sim_" + i + ".csv", "w");
                ...
                int[][] vesselSetUp = model.readVesselInitialisation(inputFolder + "VesselLocations\\vesselLocationsDensityTest_Q1.csv", nVtemp);
                ...
                int initialStromaCount = model.countInitialStroma(inputFolder + "GrowCancer\\stromaInitialCountDensityTest_" + nVtemp + ".csv");
                //double[][] initialStroma = model.initialStromaSetUp(inputFolder + "\\GrowCancer\\locationsStroma.csv", initialStromaCount);
                double[][] initialStroma = model.initialPassiveReactiveStromaSetUp(inputFolder + "GrowCancer\\locationsStromaDensityTest_" + nVtemp + ".csv", initialStromaCount);

                // determine position and age of initial cancer population
                int initialCancerCount = model.countInitialCancer(inputFolder + "GrowCancer\\cancerInitialCountDensityTest_" + nVtemp + ".csv");
                double[][] initialCancer = model.initialCancerSetUp(inputFolder + "GrowCancer\\locationsCancerDensityTest_" + nVtemp + ".csv", initialCancerCount);

                // determine intial proliferation signal over domain
                double[] proliferationSignalInit = model.initialProliferationSignal(inputFolder + "GrowCancer\\valuesProliferationSignalDensityTest_" + nVtemp + ".csv", x, y);
...
*/
                ...
                for (int j = 0; j < timesteps; j++) {
                    ...
                    double [] quadrantCancerCn = model.cancerCountQuadrants(x, y, CANCER, QUIESCENTCANCER);
                    ...
                    cancerPopulationQ1[j] = quadrantCancerCn[0];
                    cancerPopulationQ2[j] = quadrantCancerCn[1];
                    cancerPopulationQ3[j] = quadrantCancerCn[2];
                    cancerPopulationQ4[j] = quadrantCancerCn[3];
...
*/
                ...
                model.quadrantPopulationData(timeVector, cancerPopulationQ1, cancerPopulationQ2, cancerPopulationQ3, cancerPopulationQ4, quadrantPopulations);
